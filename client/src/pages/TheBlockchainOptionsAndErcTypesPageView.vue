...
<template>
  <div class="relative w-full flex flex-col gap-4 mt-10 font-medium">
    <h1 class="text-4xl font-semibold">
      {{
        routeName == "ChooseERCTypesPage" ? "Choose Type" : "Choose Blockchain"
      }}
    </h1>
    <p
      v-if="routeName == 'ChooseERCTypesPage'"
      class="text-gray-400 dark:text-darkTheme-text font-normal max-w-sm"
    >
      Choose “Single” for one of a kind or “Multiple” if you want to sell one
      collectible multiple times
    </p>
    <p class="text-gray-400 dark:text-darkTheme-text font-normal" v-else>
    ...
    >
      <div
        ...
        :value="erc.id"
        :title="erc.name"
        class="w-full flex flex-col justify-center items-center border dark:border-darkTheme-border dark:hover:border-darkTheme-hover-b rounded-xl p-8 cursor-pointer transition-all duration-300"
      >
    ...
    <div class="flex flex-row flex-wrap gap-4" v-else>
      <div
        ...
        class="flex flex-col justify-center items-center border dark:border-darkTheme-border rounded-xl p-4 cursor-pointer"
      >
